...
def get_parser():
    ...
    parser.add_argument('--summary_path', default='./output/MobileDistRes1-07152157/summary', help='the summary file save path')
    parser.add_argument('--ckpt_path', default='./output/MobileDistRes1-07152157/ckpt', help='the ckpt file save path')
    parser.add_argument('--log_file_path', default='./output/MobileDistRes1-07152157/logs', help='the ckpt file save path')
    ...
    parser.add_argument('--tau', dest='tau', default=1.0, help='KD method. tau stands for temperature.', type=float)
    parser.add_argument('--lamda', dest='lamda', default=0, help='KD method. lamda between original loss and soft-target loss.', type=float)
    ...
    parser.add_argument('--student_model_path', default='./pretrained/model.ckpt-1450000', type=str, help='restored student model path')
...
if __name__ == '__main__':
    ...
    with graph_s.as_default():
        ...
        t_tau_input = tf.placeholder(name='teacher_tau_input_to_student', shape=[None, args.num_output], dtype=tf.float32)
        ...
        soft_loss_logit = tf.reduce_mean(tf.scalar_mul(0.5, tf.square(s_tau-t_tau_input)))
        inference_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logit_s, labels=labels_s))
        total_loss = args.lamda*inference_loss + (1-args.lamda)*soft_loss_logit
    ...
    for i in range(args.epoch):
        ...
        while True:
            try:
                ...
                start = time.time()
                tau_input, acc_t_val = sess_t.run([t_tau, acc_t], feed_dict=feed_dict_t)
                
                feed_dict_s = {images_s: images_train, labels_s: labels_train, t_tau_input: tau_input}
                _, total_loss_val, inference_loss_val, soft_loss_val, _, acc_s_val = \
                    sess_s.run([train_op, total_loss, inference_loss, soft_loss_logit, inc_op, acc_s],
                              feed_dict=feed_dict_s)
                end = time.time()
                pre_sec = args.batch_size/(end - start)
                # print training information
                if count > 0 and count % args.show_info_interval == 0:
                    print('epoch %d, total_step %d, total loss is %.2f , inference loss is %.2f, soft '
                          'loss is %.2f, training accuracy is %.6f, teacher accuracy is %.6f, time %.3f samples/sec' %
                          (i, count, total_loss_val, inference_loss_val, soft_loss_val, acc_s_val, acc_t_val, pre_sec))
                count += 1
                ...
                if count > 0 and count % args.ckpt_interval == 0:
                    filename = 'MobileDistRes_iter_{:d}'.format(count) + '.ckpt'
